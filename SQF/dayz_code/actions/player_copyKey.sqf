...
if (!_haskey) exitWith {dayz_actionInProgress = false; format[localize "str_player_30",_text] call DZE_fnc_rollingMessages;};

_hastoolweapon = "ItemKeyKit" in weapons player;
if (!_hastoolweapon) exitWith {dayz_actionInProgress = false; localize "str_epoch_player_57" call DZE_fnc_rollingMessages;};

_pos = [player] call FNC_GetPos;
_isNear = {inflamed _x} count (_pos nearObjects 3);
if (_isNear == 0) exitWith {dayz_actionInProgress = false; localize "str_epoch_player_58" call DZE_fnc_rollingMessages;};

call gear_ui_init;

// require one tin bar per key
_hasTinBar = 	"ItemTinBar" in magazines player;
if (!_hasTinBar) exitWith {dayz_actionInProgress = false; localize "str_epoch_player_59" call DZE_fnc_rollingMessages;};

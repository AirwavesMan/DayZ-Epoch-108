...
if(dayz_fishingInprogress) exitWith { localize "str_fishing_inprogress" call DZE_fnc_rollingMessages;};
...
if (!_isOk) exitWith {
    ...
    localize "str_fishing_watercheck" call DZE_fnc_rollingMessages;
};
...
while {_isOk} do {
    if(dayz_isSwimming) exitWith {localize "str_player_26" call DZE_fnc_rollingMessages;_isOk = false;};
    if !((currentWeapon player) in Dayz_fishingItems or (player != vehicle player and !((vehicle player) isKindOf "Ship"))) exitwith {
        localize "str_fishing_canceled" call DZE_fnc_rollingMessages;
        _isOk = false;
    };
    if (r_interrupt) then {
        _isOk = false;
        ...
        if ((random _rnd) <= 5) then {
            ...
            if (_inVehicle) then { 
                ...
                format[localize "str_fishing_success",_text] call DZE_fnc_rollingMessages;
            } else {
                call gear_ui_init;
            ...
            switch (true) do {
                case (_counter == 0) : { format[localize "str_fishing_cast",_num] call DZE_fnc_rollingMessages; };
                case (_counter == 4) : { localize "str_fishing_pull" call DZE_fnc_rollingMessages; player playActionNow "GesturePoint"; };
                case (_counter == 8) : { localize "str_fishing_pull" call DZE_fnc_rollingMessages; player playActionNow "GesturePoint"; };
                default { localize "str_fishing_nibble" call DZE_fnc_rollingMessages; };
            }; 
            _counter = _counter + 1;
            ...
            if (_counter == 12) then {
                _isOk = false;
                uiSleep 1;
                localize "str_fishing_failed" call DZE_fnc_rollingMessages;
            };
        };
    };
};

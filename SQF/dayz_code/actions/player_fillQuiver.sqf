...
if (_qtyFreeSlots > 0) then {								// quiver is not full
	if (_arrows == 0) exitWith {							// player has no single arrows
		localize "str_quiver_notenougharrows" call DZE_fnc_rollingMessages;	// You do not have enough arrows.
	};
	...
	format [localize "str_quiver_addarrows", _qty_arrows2add] call DZE_fnc_rollingMessages;	// You have added %1 arrows to the quiver.
} else {
	localize "str_quiver_full" call DZE_fnc_rollingMessages;					// The quiver is full.
};

	...
	if (_exit) then {
		...
		[_lastSave, _startTime, _ctType, _cTarget] spawn {
			...
			if (!dialog) then {
				...
				if ((((_this select 2) in DZE_isNewStorage) || {(_this select 2) isKindOf 'DZE_Storage_Base'} || {(_this select 2) isKindOf 'Land_A_tent'} || {(_this select 2) isKindOf 'WeaponHolder'}  || {((!alive (_this select 3)) && {(_this select 2) isKindOf 'Man'})}) && {!(locked (_this select 3))}) then {
